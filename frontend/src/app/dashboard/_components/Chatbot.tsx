...
import { SendIcon, UserIcon, SparklesIcon } from '@/components/icons';
...
export const Chatbot: React.FC = () => {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      const modelMessage: ChatMessage = { role: 'model', content: response.data.response };
  ...
  return (
    <div className="flex flex-col h-full max-w-3xl mx-auto">
      <h2 className="text-2xl lg:text-3xl font-bold text-text-primary mb-4 p-4 text-center">AI Financial Chatbot</h2>
      <div className="flex-grow overflow-y-auto p-4 space-y-6">
        {messages.map((msg, index) => (
          <div key={index} className={`flex items-start gap-3 sm:gap-4 ${msg.role === 'user' ? 'justify-end' : ''}`}>
            ...
            <div className={`max-w-xs sm:max-w-md p-3 sm:p-4 rounded-2xl ${msg.role === 'user' ? 'bg-primary text-white rounded-br-none' : 'bg-surface text-text-primary rounded-bl-none'}`}>
              <div className="prose prose-sm sm:prose-base prose-invert max-w-none text-white" dangerouslySetInnerHTML={{__html: msg.content}} />
            </div>
